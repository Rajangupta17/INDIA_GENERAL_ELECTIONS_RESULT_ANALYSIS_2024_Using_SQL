...
---What are the total number of seats avilable for Elections in each state
...
---Inadian National Developmental Inclusive Alliance (I.N.D.I.A.)
SELECT 
    SUM(CASE 
            WHEN party IN (
...
-- change the value an got your data 
ORDER BY won DESC
...
---- Winning Candidate's Name .their party name ,Total votes annd the margin of victory for a spacific state
------and constituency ?
...
INNER JOIN statewise_results sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
INNER JOIN states s ON sr.State_ID = s.State_ID
WHERE cr.Constituency_Name ='GORAKHPUR'


--- What is the distribution of EVM votes Postel votes for candidates in a spacific constituency?
SELECT 
    cd.EVM_Votes,
    cd.Postal_Votes,
    cd.Total_Votes,
...
----Which candidate won and which candidate was the runner-up in each constituency 
---of State for the 2024 elections?
...
---For the state of Uttar Pradesh, what are the total number of seats, total number of candidates, total number
